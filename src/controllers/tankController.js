...
const tankController = {
  ...
  updateTank: async (req, res) => {
    try {
      ...
      const updated = await firestoreService.updateTank(id, tankData);
      if (!updated) {
  ...
  deleteTank: async (req, res) => {
    try {
      const { id } = req.params;
      const deleted = await firestoreService.deleteTank(id);
      if (!deleted) {
        return res.status(404).send({ message: 'Estanque no encontrado.' });
      }
  ...
  createTank: async (req, res) => {
    try {
      ...
      if (!estanqueId || estanqueId.trim() === '') { // Añadir trim() para IDs vacíos
        const nextTankNumber = await firestoreService.getNextTankNumber();
      ...
      const newTank = await firestoreService.createTank(estanqueId, { nombre, descripcion, capacidad });
